...
> **1.4（当前版本）**
```
这一版把国标算番从头核对了一遍，修掉一批算错。

• 国标算番对齐官方竞赛算番器。拿北大 Botzone 的官方算番器当基准，随机跑了 215 万副牌逐副比对，现在总分完全一致。之前有一批番种算错、漏算或者重复计，这版都修了。
• 「已和」的分数修好了。以前在不知道你和的是哪一张时，九莲宝灯、全求人、边张/坎张/单钓将这些番一个都算不出来——九莲宝灯会少 61 分，全求人少 6 分还会被误判成「不够起和」。
• 抢杠和、和绝张、杠上开花不再凭空加分。牌面上根本不可能成立的时候（比如你和的那张自己手里还留着），勾了也不会算，这几个勾选项现在也不显示了。
• 圈风 / 门风 挪到主页。这两个每局都在变——门风每局转一位，四局一圈换圈风——埋在设置页里等于没人会去改，而它最多能影响 4 分。现在主页点一下就能选，还有「下一局」自动轮转；手里有风刻时会提示当前按什么风位在算。
• 番型一览补上「明暗杠」。
• 旧版本里有两项国标规则的默认值和官方对不上，升级时会自动纠正，你自己改过的设置不受影响。
```
